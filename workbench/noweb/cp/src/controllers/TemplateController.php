...
use \Noweb\Cp\Themes;
use \Noweb\Cp\Widget,\Noweb\Cp\Template,\Noweb\Cp\WidgetService;
...
class TemplateController extends BaseController {
	...
	public function details($id)
	{
        ...
        $themeService = new LayoutService();
        $widgetService = new WidgetService();
        $widgetTypes = $widgetService->getWidgetType();
	    $themeService->setTheme($id);
	    $themeInfo = $themeService->getThemeInfo();

	    $widgets = $themeService->loadWidgets();

	    $pages = $themeService->loadPages();
	    $layouts = $themeService->loadLayouts();
	    $positions = $themeService->loadAllPositions();

	    $this->layout->meta = ['title' => 'Chi tiết giao diện'];
	    $this->layout->content = \View::make($this->default_view)
	    ->with([
	        'themeInfo' => $themeInfo,
	        'widgets' => $widgets,
