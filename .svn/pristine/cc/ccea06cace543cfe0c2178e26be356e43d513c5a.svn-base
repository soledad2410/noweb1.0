 ...
 * @return string
 */
function resolveUrl($baseUrl,$webDir = null) {
    $fileSystem =& CKFinder_Connector_Core_Factory::getInstance("Utils_FileSystem");
    //var_dump($fileSystem->getDocumentRootPath()) ;
    $dir = $fileSystem->getDocumentRootPath() ;
    if ($webDir != null) {
        $dir=$fileSystem->getDocumentRootPath().$webDir ;
    }
    return $dir . $baseUrl;
}
